...
fn main() -> std::io::Result<()> {
    let output_path = "data/synthetic/fan_rpm1200.dat";
    let truth_path = "data/synthetic/fan_rpm1200_truth.json";
    ...
    for _ in 0..total_events {
        ...
        // Pick a random blade
        let blade_idx = (rand() * blade_count as f32) as usize;
        ...
        let r = 50.0 + rand() * (radius - 50.0);
        
        // Add some noise to position
        let jitter_x = (rand() - 0.5) * 2.0; // +/- 1px
        let jitter_y = (rand() - 0.5) * 2.0;
        ...
        let x_u32 = x as u32;
        let y_u32 = y as u32;
        let polarity = if rand() > 0.5 { 1 } else { 0 };
        ...
        if current_time_us % 10_000 == 0 {
            truth_entries.push(format!(
                "{{\"time\": {:.4}, \"angle\": {:.4}, \"rpm\": {:.1}, \"centroid_x\": {:.1}, \"centroid_y\": {:.1}}}",
                t_secs, base_angle % (2.0 * PI), rpm, center_x, center_y
            ));
        }
    }
    ...
    println!("Done. Generated {} events.", total_events);
    Ok(())
}
